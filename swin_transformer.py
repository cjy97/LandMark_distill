...
# from core.model.dolg_model import CosineProduct
# from core.model.dolg_model import MarginCosineProduct, MarginCosineProduct_NoWeight
# from core.parameter_multicenter import ParamMultiCenter
...
from timm.models.vision_transformer import Mlp  #, PatchEmbed  #, _init_vit_weights


# from loss.circle_loss import *
...
default_cfgs = {
    ...
    'swin_base_patch4_window12_384_in22k_multilabel': _cfg(
        url='https://github.com/SwinTransformer/storage/releases/download/v1.0.0/swin_base_patch4_window12_384_22k.pth',
        input_size=(3, 384, 384), crop_pct=1.0, num_classes=21841),

    'swin_base_patch4_window12_384_in22k_cosfc': _cfg(
        url='https://github.com/SwinTransformer/storage/releases/download/v1.0.0/swin_base_patch4_window12_384_22k.pth',
        input_size=(3, 384, 384), crop_pct=1.0, num_classes=21841),
    ...
    # custom_model by paulhliu
    'swin_base_patch4_window12_480': _cfg(
        url='',
        input_size=(3, 480, 480), crop_pct=1.0),
...
class PatchReduce(nn.Module):
    ...
    def forward(self, x):
        ...
        """
        B, L, C = x.shape
        x = x.reshape(B, L // 4, C * 4)
        p = nn.AdaptiveAvgPool1d(C*2)
        x = p(x)
...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
                 ...
                 use_checkpoint=False, weight_init='', ml=False, cos_fc=False, cos_scale=8, cos_margin=0,
                 scoremap=False, one_classifier=False, **kwargs):
        super().__init__()
        ...
        #     fc_bias = False
        fc_bias = False
        self.head = nn.Linear(self.num_features, num_classes, bias=fc_bias) if num_classes > 0 else nn.Identity()
        if self.cos_fc:
            self.head = MarginCosineProduct(self.num_features, num_classes, scale=cos_scale, m=cos_margin)
        self.head_singlelabel = nn.Identity()
        self.token_attn = nn.Identity()
        if self.ml:
            self.head_singlelabel = nn.Linear(self.num_features, num_classes, bias=False)
            self.token_attn = nn.Conv2d(self.num_features, num_classes, (1, 1), bias=False)
    ...
    def forward_features(self, x):
        ...
        x = self.norm(x)  # B L C
        if self.ml:
            feat = self.avgpool(x.transpose(1, 2))  # B C 1
            feat = torch.flatten(feat, 1)
            if self.one_classifier:
                return feat, x
            if self.scoremap:
                score_map = self.head_singlelabel(x)
    ...
    def forward(self, x, label):
        if self.ml:
            feat, token_feat = self.forward_features(x)
            x = self.head(feat)
            return x, token_feat
        else:
...
def _create_swin_transformer(variant, pretrained=False, ml=False, cos_fc=False, default_cfg=None, **kwargs):
    ...
    model = SwinTransformer(img_size=img_size, num_classes=num_classes, ml=ml, cos_fc=cos_fc, **kwargs)
...
# def _create_swin_transformer_dense(variant, pretrained=False, ml=False, default_cfg=None, **kwargs):
#     if default_cfg is None:
#         default_cfg = deepcopy(default_cfgs[variant])
#     # overlay_external_default_cfg(default_cfg, kwargs)
#     default_num_classes = default_cfg['num_classes']
...
#     model = SwinTransformer_Dense(img_size=img_size, num_classes=num_classes, ml=ml, **kwargs)
#     model.default_cfg = default_cfg
...
# @register_model
# def swin_base_patch4_window12_384_in22k_dense(pretrained=False, **kwargs):
#     """ Swin-B @ 384x384, trained ImageNet-22k
#     """
#     model_kwargs = dict(
#         patch_size=4, window_size=12, embed_dim=128, depths=(2, 2, 18, 2), num_heads=(4, 8, 16, 32), **kwargs)
#     return _create_swin_transformer_dense('swin_base_patch4_window12_384_in22k', pretrained=pretrained, **model_kwargs)


@register_model
def swin_base_patch4_window12_384_in22k_multilabel(pretrained=False, **kwargs):
    """ Swin-B @ 384x384, trained ImageNet-22k
    """
    model_kwargs = dict(
        patch_size=4, window_size=12, embed_dim=128, depths=(2, 2, 18, 2), num_heads=(4, 8, 16, 32), **kwargs)
    return _create_swin_transformer('swin_base_patch4_window12_384_in22k_multilabel', pretrained=pretrained, ml=True, **model_kwargs)


@register_model
def swin_base_patch4_window12_384_in22k_cosfc(pretrained=False, **kwargs):
    """ Swin-B @ 384x384, trained ImageNet-22k
    """
    model_kwargs = dict(
        patch_size=4, window_size=12, embed_dim=128, depths=(2, 2, 18, 2), num_heads=(4, 8, 16, 32), **kwargs)
    return _create_swin_transformer('swin_base_patch4_window12_384_in22k_cosfc', pretrained=pretrained, cos_fc=True, **model_kwargs)
...
@register_model
def swin_base_patch4_window12_480(pretrained=False, **kwargs):
    """ Swin-B @ 384x384, pretrained ImageNet-22k, fine tune 1k
    """
    model_kwargs = dict(
        patch_size=4, window_size=12, embed_dim=128, depths=(2, 2, 18, 2), num_heads=(4, 8, 16, 32), **kwargs)
    return _create_swin_transformer('swin_base_patch4_window12_480', pretrained=pretrained, **model_kwargs)
